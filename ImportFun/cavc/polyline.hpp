	...
	template <typename Real> AABB<Real> getExtents(Polyline<Real> const& pline) {
		...
		auto visitor = [&](std::size_t i, std::size_t j) {
			...
			if (v1.bulgeIsZero()) {
				...
				if (v1.bulgeIsNeg()) { //fixed miscalculation of clockwise arc! by wang huachang, v2022.05.02
					Real tmpval = startAngle;
					startAngle = endAngle, endAngle = tmpval;
					sweepAngle = utils::deltaAngle(startAngle, endAngle); //v2022.06.30 by liyifan
				}
